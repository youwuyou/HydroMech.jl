...
##################################################
@views function PorosityWave2D(;t_tot_, COMPRESSIBLE = false)
    ...
    if COMPRESSIBLE 
        pt = PTCoeff(OriginalDamping, mesh, μˢ)        # choose this norm does not get smaller than norm_Rx=5.261e-01, dt = 15
    ...
    # Visualization
    if DO_VIZ
        gif(anim, "PorosityWave2D.gif", fps = 15)
    end   

    # Testing
    ## store data in case further testings needed
    if SAVE_TEST && COMPRESSIBLE == false
        save("../../test/2D/incom_Peff.jld", "data", Array(flow.Pt - flow.Pf)')  # store case for reference testing
    end
...
if isinteractive()
    PorosityWave2D(;t_tot_=0.02, COMPRESSIBLE = true) # for reproducing porosity wave benchmark
    # PorosityWave2D(;t_tot_=0.1) # for reproducing porosity wave benchmark
    # PorosityWave2D(;t_tot_=0.0005) # for reproducing the test result
end
