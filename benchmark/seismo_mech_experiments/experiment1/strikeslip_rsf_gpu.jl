...
@views function Stokes2D_vep()
    ...
    nt            = 5000                # small number for the first-time compilation run, set to other values for longer run
    # εnl           = 1.0e-6
    εnl           = 1.0e1
    maxiter       = 10000
    ...
    for it = 1:nt
        ...
        # @show dt = min(1.9*L*0.5/maximum(Vp), 0.1time_year)
        @show dt = min(1.9*L*0.5/(maximum(Vp)*10), 0.05time_year)
        ...
        while err > εnl && iter < maxiter
            ...
            @parallel compute_slip_rate!(Vp[:, h_index], τii[:, h_index], a[:, h_index], b[:, h_index], Ω[:, h_index], γ[:, h_index], Pt, V0)
