...
@views function HydroMech2D_compressible(;t_tot_)
    # Physics - scales
    ρfg      = 1.0             # fluid rho*g
    ...
    while t<t_tot
        ...
        @parallel update_old_compressible!(Phi_o, ∇V_o, Pt_o, Pf_o, Phi, ∇V,  Pt, Pf)
        ...
        while err > ε && iter <= iterMax
            ...
            @parallel compute_RP_compressible!(dτPf, RPt, RPf, K_muf, ∇V, ∇qD, Pt, Pf, EtaC, Phi, Kd, ɑ, Pt_o, Pf_o, B, Pfsc, Pfdmp, min_dxy2, _dx, _dy, dt)
    ...
    # Visualization
    if DO_VIZ
        gif(anim, "HydroMech2D_compressible.gif", fps = 15)
    end
...
# if isinteractive()
#     # HydroMech2D_compressible(;t_tot_=0.02) # for reproducing porosity wave benchmark
#     HydroMech2D_compressible(;t_tot_=0.03) # for R=1
#     # HydroMech2D_compressible(;t_tot_=0.1) # for reproducing porosity wave benchmark
#     # HydroMech2D_compressible(;t_tot_=0.0005) # for reproducing the test result
# end
